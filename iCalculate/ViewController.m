...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self textFieldShouldBeginEditing:self.displayText];
}
...
{
    if ([self.displayText.text isEqualToString:@"Math Error"])
    {
        self.expression = @"";
    }
    if (sender.tag != 15 /*equal sign tag*/)
    {
...
-(void)calculate:(NSString *)exp
{
    @try
    {
        if (![self isValidString:self.expression])
        {
            self.expression = @"Math Error";
        }
        else
        {
            self.expression = [[self evaluateString:self.expression] stringValue];
        }
    }
